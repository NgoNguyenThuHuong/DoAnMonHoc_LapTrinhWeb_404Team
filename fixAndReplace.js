...
// Find the start of // ===== CHARACTER MODAL =====
const startIndex1 = content.indexOf('// ===== CHARACTER MODAL =====');
if (startIndex1 !== -1) {
    // remove everything from // ===== CHARACTER MODAL ===== to the end of the file
    content = content.substring(0, startIndex1);
}

// Find the start of // ===== AI HANZI STORY DICTIONARY =====
const startIndex2 = content.indexOf('// ===== AI HANZI STORY DICTIONARY =====');
if (startIndex2 !== -1) {
    content = content.substring(0, startIndex2);
}

const newShowCharModal = `
...
content += newShowCharModal;

// Fix encoding by using buffer
const buff = Buffer.from(content, 'utf8');
fs.writeFileSync(path, buff);
console.log('Successfully replaced and fixed site.js');
